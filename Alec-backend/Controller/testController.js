...
const getCourseWithTestModules = async (req, res) => {
  ...
  try {
    const testSeries = await TestSeries.findById(req.params.id).populate('category');
    if (!testSeries) {
...
const getPreSeriesUsingCategoryId = async (req, res) => {
  try {
    console.log(req.params)
    const course = await Course.find({category: req.params.categoryId}).populate("category");
    console.log(course)
    if (!course) {
      return res.status(404).json({ message: "Course not found" });
    }
